...
async def auto_seed_if_empty():
    """Seed data automatically on startup if database has no users."""
    try:
        async with AsyncSessionLocal() as session:
            result = await session.execute(select(User))
            if result.scalars().first():
                return  # Database already seeded
        print("[AUTO-SEED] Fresh database detected. Seeding initial data...")
        await seed_data(drop_tables=False)
    except Exception as e:
        print(f"[AUTO-SEED] Notice: {e}")

async def seed_data(drop_tables=False):
    if drop_tables:
    ...
    async with AsyncSessionLocal() as session:
        ...
        for i in range(25):
            ...
            inc = Incident(
                ...
                title=f"{base[0]} #{i+1}",
                description=f"{base[1]}. Automated system alert detected at {datetime.now().strftime('%H:%M')}.",
                category=base[3],
                priority=base[2],
        ...
        for c_idx, (c_title, c_desc, c_type, c_risk, c_st, c_appr) in enumerate(changes_data):
            chg = ChangeRequest(
                ...
                approval_decision_at=datetime.now(),
        ...
        for r_idx, (r_name, r_type, ip, env, st, hl, cpu, mem, disk, net, resp, avail) in enumerate(infra_data):
            ...
            for m_idx in range(20):
                m_time = datetime.now() - timedelta(hours=(20 - m_idx))
        ...
        for a_idx, (r_id, m_name, c_val, t_val, sev, msg, st) in enumerate(alerts_config):
            alt = InfrastructureAlert(
                ...
                message=msg,
                status=st,
                created_at=datetime.now() - timedelta(hours=a_idx+1),
        ...
        for idx, (act, mod, det) in enumerate(audit_events):
            audit = AuditLog(
                ...
                timestamp=datetime.now() - timedelta(hours=idx*3)
            )
            session.add(audit)
...
if __name__ == "__main__":
    asyncio.run(seed_data(drop_tables=True))
